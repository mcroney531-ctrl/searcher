...
- **Title and description filters combine as AND, not OR.** The `size_overlap_title_and_desc` call (credit log line 15) sent `job_title_or` and `job_description_pattern_or` in one request and returned 78, which is the intersection. A true title-OR-description union therefore needs two pulls: titles first, then descriptions with `job_id_not` set to the title results.
